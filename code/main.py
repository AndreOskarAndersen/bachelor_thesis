...
class Residual(nn.Module):
    def __init__(self, in_channels = 256):
        ...
        self.branch_cov = nn.Conv2d(in_channels = in_channels, out_channels = 256, kernel_size = 3)
...
class Hourglass(nn.Module):
    def __init__(self, num_layers = 4, num_bottlenecks = 2):
        ...
        for _ in range(self.num_layers):
            self.encoder_max_poolings.append(nn.MaxPool2d(3))
            self.encoder_residuals.append(Residual())
            self.decoder_upsamplings.append(nn.Upsample())
...
class SHG(nn.Module):
    def __init__(self, num_hourglasses = 2, num_layers = 4, num_bottlenecks = 2):
        ...
        self.pre_max_pool = nn.MaxPool2d(kernel_size = 60, stride = 1) 
        ...
        for i in range(self.num_hourglasses - 1):
            self.post_conv_1.append(nn.Conv2d(17, 17, kernel_size = 1))
            self.post_conv_2.append(nn.Conv2d(17, 17, kernel_size = 1))
            self.post_blue_box_conv.append(nn.Conv2d(17, 17, kernel_size = 1))
    ...
    def forward(self, x):
        ...
        x = nn.Conv2d(17, 17, kernel_size = 1)
        x = nn.Conv2d(17, 17, kernel_size = 1)
        return x
