...
\subsubsection{Reasoning behind using the Stacked Hourglass}\label{subsec:reasoning}
...
\begin{algorithm}[htbp]
    ...
    \begin{algorithmic}[1]
        ...
        \end{varwidth}
        \While{$\bm{Z}_{nk}$ is changed between each iteration}
            \ForAll{observations $\bm{x}_n \in \bm{X}$}
